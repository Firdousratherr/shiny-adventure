...
export default function CartPage() {
  ...
  return <main className="container min-h-screen py-5 sm:py-8">
    ...
    {!items.length ? <div className="mx-auto max-w-md py-20 text-center"><div className="text-5xl">🛒</div><p className="mt-4 font-bold">Your cart is empty.</p><p className="mt-1 text-sm text-slate-500">Find something you love and add it here.</p><Link href="/products" className="mt-5 inline-block rounded-xl bg-slate-900 px-6 py-3 text-sm font-bold text-white">Shop products</Link></div> :
      <div className="mt-6 grid gap-5 lg:grid-cols-[1fr_340px]">
        <section className="space-y-3">{items.map(item => <div key={item.productId} className="flex gap-3 rounded-2xl border border-slate-200 bg-white p-3 shadow-sm">
          <Link href="/products" className="h-20 w-20 shrink-0 overflow-hidden rounded-xl bg-slate-100 sm:h-24 sm:w-24">{item.image && <img src={item.image} alt="" className="h-full w-full object-cover"/>}</Link>
          <div className="min-w-0 flex-1"><h2 className="line-clamp-2 text-sm font-bold sm:text-base">{item.name}</h2><p className="mt-1 text-sm font-black">₹{item.price.toLocaleString('en-IN', { minimumFractionDigits: 2 })}</p>
            <div className="mt-2 flex items-center gap-2"><button onClick={() => setQuantity(item.productId, item.quantity - 1)} className="h-8 w-8 rounded-lg border text-lg">−</button><span className="w-5 text-center text-sm font-bold">{item.quantity}</span><button onClick={() => setQuantity(item.productId, item.quantity + 1)} className="h-8 w-8 rounded-lg border text-lg">+</button><button onClick={() => remove(item.productId)} className="ml-1 text-xs font-bold text-red-600">Remove</button></div>
          </div>
        ...
        <aside className="h-fit rounded-2xl border border-slate-200 bg-white p-5 shadow-sm lg:sticky lg:top-20"><h2 className="text-lg font-black">Order summary</h2><div className="mt-4 flex justify-between text-sm"><span>Subtotal</span><strong>₹{subtotal.toLocaleString('en-IN', { minimumFractionDigits: 2 })}</strong></div><p className="mt-2 text-xs text-slate-500">Delivery and final total are calculated securely at checkout.</p><Link href="/checkout" className="mt-5 block rounded-xl bg-slate-900 px-5 py-3.5 text-center text-sm font-black text-white">Proceed to checkout →</Link></aside>
      </div>}
  </main>;
}