...
export default function StaffPage() {
  ...
  const [selected, setSelected] = useState<string[]>(['products', 'pricing', 'images', 'inventory']);
  ...
  const create = async (e: React.FormEvent) => {
    ...
    try {
      const r = await fetch('/api/admin/staff', {
        ...
        body: JSON.stringify({ email, password, permissions: selected }),
      });
      const j = await r.json();
      if (!r.ok) throw new Error(j.error);
      setEmail('');
  ...
  const update = async (id: string, data: Record<string, unknown>) => {
    ...
    try {
      ...
      const j = await r.json();
      if (!r.ok) throw new Error(j.error);
      setStaff((x) => x.map((s) => (s.id === id ? j.staff : s)));
    } catch (e) {
      ...
      <div className="container py-6 sm:py-8">
        {forbidden ? (
          <div className="rounded-2xl bg-white p-8 text-center shadow-sm">
            <h1 className="text-xl font-black">Super administrator access required</h1>
            <p className="mt-2 text-sm text-slate-500">
              Only the store owner can create or manage staff accounts.
            </p>
            <Link
              href="/admin/dashboard"
              className="mt-5 inline-block rounded-xl bg-slate-900 px-4 py-2.5 text-sm font-bold text-white"
            >
              Back to dashboard
            </Link>
          </div>
          ...
          <>
            ...
            <h1 className="mt-1 text-3xl font-black">Staff & permissions</h1>
            <p className="mt-2 max-w-2xl text-sm text-slate-500">
              Create separate staff logins and give each person only the store permissions they
              need. Your super-admin account keeps full access.
            </p>
            ...
            <div className="mt-6 grid gap-6 lg:grid-cols-[380px_1fr]">
              <section className="rounded-2xl bg-white p-5 shadow-sm">
                <h2 className="text-lg font-black">Create staff account</h2>
                <form onSubmit={create} className="mt-4 space-y-4">
                  ...
                  <label className="block text-sm font-bold">
                    ...
                    <span className="mt-1 block text-[11px] font-normal text-slate-500">
                      Give the staff member this password securely; they can use the admin login.
                    </span>
                  </label>
                  ...
                  <button
                    disabled={saving}
                    className="h-12 w-full rounded-xl bg-slate-900 font-black text-white disabled:opacity-50"
                  >
              ...
              <section>
                ...
                {loading ? (
                  ...
                  <div className="space-y-3">
                    {staff.map((s) => (
                      <article key={s.id} className="rounded-2xl bg-white p-4 shadow-sm sm:p-5">
                        <div className="flex flex-wrap items-start justify-between gap-3">
                          <div>
                            <p className="font-black">{s.email}</p>
                            <p className="mt-1 text-xs text-slate-500">
                              {s.accessRole === 'SUPER_ADMIN' ? 'Super administrator' : 'Staff'} ·
                        ...
                        <div className="mt-4 flex flex-wrap gap-2">
                          {permissions
                            .filter((p) => s.permissions?.includes(p.key))
                            .map((p) => (
                              <span
                                key={p.key}
                                className="rounded-full bg-slate-100 px-2.5 py-1 text-[11px] font-bold"
                              >
                                {p.label}
                              </span>
                            ))}
                        ...
                        {s.accessRole !== 'SUPER_ADMIN' && (
                          <div className="mt-4 border-t pt-4">
                            ...
                            <div className="flex flex-wrap gap-2">
                              {permissions.map((p) => {
                                const hasPermission = s.permissions.includes(p.key);
                                return (
                                  <button
                                    type="button"
                                    key={p.key}
