...
export default async function AdminOrders({ searchParams }: { searchParams?: { status?: string; q?: string } }) {
  ...
  const orders = await db.order.findMany({
    where: { deletedAt: null, ...(status ? { status: status as any } : {}), ...(q ? { OR: [{ orderNumber: { contains: q, mode: 'insensitive' } }, { customerName: { contains: q, mode: 'insensitive' } }, { phone: { contains: q } }] } : {}) },
  ...
  return <main className="min-h-screen bg-slate-100"><AdminNav active="orders"/><div className="container py-8">
    <div className="flex flex-wrap items-center justify-between gap-3"><div><p className="text-sm text-slate-500">Zenvora Admin</p><h1 className="text-3xl font-black">Order management</h1></div><div className="flex items-center gap-3"><Link href="/admin/orders/archived" className="rounded-xl border bg-white px-4 py-2 text-sm font-bold">Archived orders</Link><Link href="/admin/dashboard" className="font-semibold">← Dashboard</Link></div></div>
    ...
    <div className="mt-6 space-y-5">{!orders.length&&<div className="rounded-2xl bg-white p-8 text-center text-slate-500">No orders found.</div>}{orders.map(order=><section key={order.id} className="rounded-2xl bg-white p-6 shadow-sm ring-1 ring-slate-100">
      ...
      {order.status==='PAYMENT_PENDING'&&<PaymentActions orderNumber={order.orderNumber} hasProof={Boolean(order.paymentScreenshotUrl)}/>}<OrderActions orderNumber={order.orderNumber} status={order.status}/><DeleteOrderButton orderNumber={order.orderNumber}/>
