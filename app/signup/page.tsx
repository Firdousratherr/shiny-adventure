...
export default function Signup() {
  ...
  async function submit(e:FormEvent) {
    ...
    try {
      ...
      if(!res.ok){setError(result.error||'Unable to create your account.');setLoading(false);return;}
      ...
      <section className="flex min-h-[680px] items-center p-6 sm:p-10 lg:p-14"><div className="w-full max-w-md mx-auto">
        ...
        <p className="mt-8 text-sm font-black uppercase tracking-[.2em] text-fuchsia-400">New customer</p><h1 className="mt-2 text-4xl font-black tracking-tight">Create account</h1><p className="mt-3 text-sm leading-6 text-slate-400">Create your account with your name, email and password.</p>
          <form onSubmit={submit} className="mt-8 space-y-4">
            ...
            <button disabled={loading} className="h-14 w-full rounded-2xl bg-gradient-to-r from-violet-600 to-fuchsia-500 font-black shadow-lg shadow-violet-900/20 transition hover:-translate-y-0.5 disabled:opacity-60">{loading?'Creating account…':'Create Account →'}</button>
          </form>
