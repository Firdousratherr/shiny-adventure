...
export default function Signup() {
  ...
  const [error,setError]=useState(''); const [loading,setLoading]=useState(false); const [show,setShow]=useState(false); const [verification,setVerification]=useState(false); const [otp,setOtp]=useState('');
  ...
  async function submit(e:FormEvent) {
    ...
    } catch { setError('Unable to create your account right now.'); setLoading(false); }
  }
  ...
  return <main className="relative min-h-screen overflow-hidden bg-[#070b16] px-4 py-6 text-white sm:px-6 sm:py-10">
    ...
    <div className="relative mx-auto grid min-h-[calc(100vh-3rem)] max-w-6xl overflow-hidden rounded-[2rem] border border-white/10 bg-[#0c1222] shadow-[0_30px_100px_rgba(0,0,0,.5)] md:grid-cols-2">
      ...
      <section className="flex min-h-[680px] items-center p-6 sm:p-10 lg:p-14"><div className="w-full max-w-md mx-auto">
        ...
        <p className="mt-8 text-sm font-black uppercase tracking-[.2em] text-fuchsia-400">New customer</p><h1 className="mt-2 text-4xl font-black tracking-tight">Create account</h1><p className="mt-3 text-sm leading-6 text-slate-400">{verification ? <>Enter the 6-digit code sent to <b className="text-slate-200">{email}</b>.</> : 'Create your account with your name, email and password.'}</p>
          ...
          <form onSubmit={submit} className="space-y-4">
            ...
            {verification && <label className="block text-sm font-bold text-slate-200">Verification code<input required inputMode="numeric" pattern="[0-9]{6}" maxLength={6} value={otp} onChange={e=>setOtp(e.target.value.replace(/\D/g,''))} autoComplete="one-time-code" placeholder="123456" className="mt-2 h-14 w-full rounded-2xl border border-white/10 bg-white/5 px-4 text-center text-2xl font-black tracking-[.5em] text-white outline-none placeholder:text-slate-600 focus:border-violet-500 focus:ring-4 focus:ring-violet-500/10"/></label>}
            ...
            <button disabled={loading} className="h-14 w-full rounded-2xl bg-gradient-to-r from-violet-600 to-fuchsia-500 font-black shadow-lg shadow-violet-900/20 transition hover:-translate-y-0.5 disabled:opacity-60">{loading ? (verification ? 'Verifying…' : 'Sending code…') : (verification ? 'Verify & Create Account →' : 'Send Verification Code →')}</button>
            {verification && <button type="button" onClick={()=>{setVerification(false);setOtp('');setError('')}} className="w-full text-sm font-bold text-slate-400 hover:text-white">← Change email or details</button>}
          </form>
