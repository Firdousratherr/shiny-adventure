...
export async function POST(request: Request) {
  try {
    ...
    const subtotal = data.items.reduce((sum, item) => sum.plus(byId.get(item.productId)!.sellingPrice.mul(item.quantity)), new (require('@prisma/client').Prisma.Decimal)(0));
    ...
    const shipping = deliveryCharge(subtotal, values.freeShippingThreshold || '99900', values.flatDeliveryCharge || '7900');
