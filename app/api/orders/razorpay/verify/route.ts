...
export async function POST(request: Request) {
  try {
    ...
    if (payment.order_id !== razorpayOrderId || payment.status !== 'captured' || Number(payment.amount) !== Math.round(Number(order.totalAmount) * 100)) {
      return NextResponse.json({ error: 'Razorpay payment is not captured for the expected amount.' }, { status: 400 });
    }
    ...
    const result = await db.$transaction(async tx => {
      ...
      // Inventory was already deducted when the order reservation was created.
