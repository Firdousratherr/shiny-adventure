...
export async function POST(request: Request) {
  try {
    ...
    if (typeof orderNumber !== 'string' || !/^ORD-\d{4}-\d{4,}$/i.test(orderNumber.trim())) return NextResponse.json({ error: 'A valid order number is required.' }, { status: 400 });
    const normalizedOrderNumber = orderNumber.trim().toUpperCase();
    const order = await db.order.findUnique({ where: { orderNumber: normalizedOrderNumber }, select: { id: true, orderNumber: true, totalAmount: true, status: true, razorpayOrderId: true } });
    ...
    await db.order.update({ where: { id: order.id, status: 'PAYMENT_PENDING', razorpayOrderId: null }, data: { razorpayOrderId: rpOrder.id, paymentMethod: 'RAZORPAY' } });
