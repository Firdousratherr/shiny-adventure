...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (!existing) {
      try {
        await db.razorpayWebhookEvent.create({ data: { eventId, event, payload } });
      } catch (error) {
        // Another delivery may have inserted the same event between the read and create.
    ...
    await db.$transaction(async tx => {
      ...
      if (lockedEvent[0].processedAt) return;
    ...
    // A non-2xx response tells Razorpay to retry transient processing failures.
