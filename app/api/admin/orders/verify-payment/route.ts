...
import { orderStatusTransitions } from '../../../../../lib/orders/status';
  ...
  try {
    ...
    const result = await db.$transaction(async (tx) => {
      ...
      const updatedOrder = await tx.order.update({
        where: { id: order.id },
        data: { status: 'CONFIRMED', paymentVerifiedAt: new Date(), paymentVerifiedBy: session.user.email, paymentVerificationNote: note || null, paymentRejectionReason: null },
