...
export async function GET(request:Request){
 ...
 try{const result=await get(order.paymentScreenshotUrl);if(!result)return NextResponse.json({error:'Payment proof not found.'},{status:404});return new NextResponse(result.stream,{status:200,headers:{'Content-Type':result.blob.contentType||'application/octet-stream','Cache-Control':'private, no-store','Content-Disposition':'inline'}})}catch(error){console.error('payment proof access failed',error);return NextResponse.json({error:'Unable to load payment proof.'},{status:500})}
}
